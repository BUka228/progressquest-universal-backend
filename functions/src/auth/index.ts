...
export const processNewUser = functionsV1.auth.user()
  .onCreate(async (user) => {
    ...
    const gamificationProfileRef =
      db.collection(GAMIFICATION_PROFILES_COLLECTION).doc(uid);
    const globalStatsRef =
      db.collection(GLOBAL_STATISTICS_COLLECTION).doc(uid);
    const personalWorkspaceRef =
      db.collection(WORKSPACES_COLLECTION).doc();
